import{useEffect as y,useRef as _}from"react";import S from"../../hooks/useMediaQuery.js";var f="7ac4fdabc036c66049ebb98d7a60c135c0f0b9e8fb43bb207cb76cfbd7e3fefd",g=`._wrapper_qd4p2_1 {
  position: relative;
  width: fit-content;
  height: fit-content;
}
...
`,x=(t,e,r=800)=>{let a=[{transform:"scale(0.5)"},{transform:"scale(1)"}],n=[{opacity:0},{opacity:1},{opacity:0}],s={duration:r,fill:"forwards"};e.animate(n,s),t.animate(a,s)},E=(t=5,e=15)=>r=>{let a=Math.floor(Math.max(Math.random()*e,t));r.style.width=`${a}px`,r.style.height=`${a}px`,r.style.top=`${Math.floor(Math.random()*70)}%`,r.style.left=`${Math.floor(Math.random()*100)}%`},C=()=>{let t=document.createElement("div"),e=document.createElement("div");return t.style.position="absolute",t.classList.add(p.star),e.classList.add(p.startSvgWrapper),e.innerHTML=w,t.appendChild(e),[t,e]},T=(t,{numOfStars:e=10,duration:r=800,minSize:a=10,maxSize:n=20})=>{let s=r,m=3013,i=[],c=[];for(let o=0;o<e;o++){let u=o*(r+o*10),[l,h]=C();c.push(l),t?.appendChild(l);let v=E(a,n);i.push(setInterval(()=>{v(l),x(l,h,r)},u))}return()=>{i?.forEach(o=>clearInterval(o)),c.forEach(o=>{t?.removeChild(o)})}},W=({children:t,minSize:e=10,maxSize:r=20,numOfStars:a=5,duration:n=600,color:s="#debd05e"})=>{let m=_(null),i=S("(prefers-reduced-motion: reduce)");return y(()=>{let c=!i&&T(m?.current,{numOfStars:a,duration:n,minSize:e,maxSize:r});return()=>{typeof c=="function"&&c()}},[i,e,r,a,n]),L("div",{className:p.wrapper,style:{"--path-fill":s},ref:m,children:t})},$={children:b(),minSize:d(10,!0,2),maxSize:d(10,!0,2),numOfStars:d(20,!1,5,{min:1,max:100}),duration:d(.5,!1,.1,{min:.1,max:2}),color:{type:M.Color,defaultValue:"#debd05e"}};export{W as Sparkles,$ as propsControls};
