import{useEffect as v,useRef as g}from"react";import _ from"../../hooks/useMediaQuery.js";var m="887759b697b20ebccfafc14e2e8edc5476e94d1768e6432d78170a553ba5b47e",u=`._wrapper_tnefc_1 {
  position: relative;
  width: fit-content;
  height: fit-content;
  min-width: 20px;
...
`,x=(t,e,r=800)=>{let a=[{transform:"scale(0.5)"},{transform:"scale(1)"}],s=[{opacity:0},{opacity:1},{opacity:0}],o={duration:r,fill:"forwards"};e.animate(s,o),t.animate(a,o)},M=(t=5,e=15)=>r=>{let a=Math.floor(Math.max(Math.random()*e,t));r.style.width=`${a}px`,r.style.height=`${a}px`,r.style.top=`${Math.floor(Math.random()*70)}%`,r.style.left=`${Math.floor(Math.random()*100)}%`},b=()=>{let t=document.createElement("div"),e=document.createElement("div");return t.style.position="absolute",t.classList.add(p.star),e.classList.add(p.starSvgWrapper),e.innerHTML=w,t.appendChild(e),[t,e]},E=(t,{numOfStars:e=10,duration:r=800,minSize:a=10,maxSize:s=20})=>{let o=[],i=[];for(let n=0;n<e;n++){let c=n*(r+n*10),[l,f]=b();i.push(l),t?.appendChild(l);let h=M(a,s);o.push(setInterval(()=>{h(l),x(l,f,r)},c))}return()=>{o?.forEach(n=>clearInterval(n)),i.forEach(n=>{t?.removeChild(n)})}},I=({children:t,minSize:e=20,maxSize:r=25,numOfStars:a=5,duration:s=600,color:o="#debd05"})=>{let i=g(null),n=_("(prefers-reduced-motion: reduce)");return v(()=>{let c=!n&&E(i?.current,{numOfStars:a,duration:s,minSize:e,maxSize:r});return()=>{typeof c=="function"&&c()}},[n,e,r,a,s]),C("div",{className:p.wrapper,style:{"--path-fill":o},ref:i,children:t})},R={children:y(),minSize:d(10,!0,2),maxSize:d(10,!0,2),numOfStars:d(20,!1,5,{min:1,max:100}),duration:d(.5,!1,.1,{min:.1,max:2}),color:{type:S.Color,defaultValue:"#debd05e"}};export{I as Sparkles,R as propsControls};
